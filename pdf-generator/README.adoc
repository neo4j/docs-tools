...
A docset using `stem`/`latexmath` blocks (via `@djencks/asciidoctor-mathjax`,
registered in its own `preview.yml` for the live HTML site) gets the same
math typeset in its PDF automatically, with nothing extra to configure -
`scripts/convert.js` registers the adapter in `extensions/` for the final
PDF conversion whenever that package is present in the docset's own
install. A docset that doesn't use it is unaffected.
...
- `scripts/convert.js` - wraps the real `asciidoctor-web-pdf` binary as the
  ...
  any docset that doesn't. Also passes `-B <docdir>` (the same docdir the
  assembler passes as `-a docdir=...`) through to the renderer - reading the
  merged `.adoc` from stdin, asciidoctor-web-pdf invents a fictive input
  path rooted at `-B` if given, else its own cwd, and writes its temporary
  intermediate HTML file next to that path, not next to the real docdir.
  Every image target resolves correctly (relative to the real docdir), but
  without `-B` the browser loads that temp HTML from the wrong directory,
  so every image in the PDF silently fails to resolve.
...
- `extensions/` - `roles-labels-postprocessor.js` and
  ...
  `mathjax-adapter.js` is the same idea for `@djencks/asciidoctor-mathjax`
  (`stem`/`latexmath` block support), but unlike the two above it isn't a
  dependency of this package - it's optional, registered in
  `scripts/convert.js` only when the docset's own install actually has that
  package (i.e. the docset added it itself for the live HTML site, same as
  today). Its incompatibilities with 4.x go well beyond one call site
  (`doc.$playback_attributes`/`$restore_attributes`,
  `processor.$create_pass_block`, `stem.attributes.$$smap.role`, a bare
  `Opal` global that doesn't exist in this non-Opal engine at all,
  `cell.$inner_document()`), so rather than chase each one with a `this`
  proxy like the other two adapters, `mathjax-adapter.js` loads the real
  package's source and rewrites those call sites in memory before
  evaluating it - see its own comment.
  `keep-image-with-intro-postprocessor.js` wraps an image together with a
  paragraph that directly introduces it (e.g. "Here's an image:") in a
  `.keep-with-image` div, so `print.css` can give that pair
  `break-inside: avoid-page` - if the image doesn't fit in the remaining
  space on a page, both move to the next page together instead of the
  intro text being left stranded on the page before. This has to be real
  HTML restructuring, not a `print.css` rule reaching for the image
  directly with `:has()` - confirmed directly that this renderer's
  Vivliostyle Viewer layout engine doesn't evaluate `:has()` for
  fragmentation, or at all (a `:has()` rule that should have matched didn't
  even apply its own unrelated declarations, like a plain background
  colour).
  `colophon-postprocessor.js` inserts a one-time copyright/trademark
  colophon page right after the cover/title page and before the table of
  contents (see "Known limitations" below on why this exists instead of
  running `@neo4j-antora/mark-terms`). Inserted as HTML directly before
  whichever element the converter actually produced for the TOC (or, with
  no TOC, the main content div) - not as real AsciiDoc content in the
  document preamble, which would risk pulling a docset's own genuine intro
  text (content before its first page's own first heading is also
  "preamble") in front of the TOC along with it.
...
- Classification of `reusable-docs-build.yml`'s default `antora-extensions`
  ...
  not a rendering gap.
- **Decided against** running `@neo4j-antora/mark-terms` in this pipeline.
  It *is* a plain Asciidoctor extension (`module.exports = function
  (registry) {...}`, no Antora event hook) - structurally it'd just need
  `--extension` registration, the same easy fix as `macros`/
  `remote-include`. But mark-terms adds a trademark/copyright mark on a
  term's *first use per page* (e.g. the first "Neo4j" on each HTML page) -
  a PDF has no equivalent notion of "page" the way a docset's HTML pages
  do, so "first use" has no sound definition here (per PDF? per chapter?
  just once, ever?). Went with a single boilerplate copyright/trademark
  colophon page instead - see `colophon-postprocessor.js` below.
